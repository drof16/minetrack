...
For Tailscale access from another device, run the server with:
...
After running migrations, refresh the `minetrack` database in HeidiSQL. You should see tables such as `users`, `items`, `customers`, `mines`, `orders`, `order_items`, `invoices`, `payments`, `settings`, `categories`, `locations`, and `payment_methods`.
